...
export default function StandingsPage() {
  ...
  useEffect(() => {
    ...
    fetch(csvUrl)
      ...
      .then((text) => {
        ...
        // Assuming your CSV has a "type" column with "driver" or "constructor"
  ...
  const buttonBase = { padding: "0.5rem 0.9rem", borderRadius: "8px", cursor: "pointer", border: "1px solid #6B46C1", fontWeight: 600, marginRight: "0.6rem" };
  const activeBtn = { background: "#6B46C1", color: "#fff" };
  const inactiveBtn = { background: "#fff", color: "#6B46C1" };
  const cardStyle = { background: "#fff", borderRadius: "12px", boxShadow: "0 6px 18px rgba(0,0,0,0.08)", overflow: "hidden", marginTop: "1rem" };
  const tableWrapper = { overflowX: isMobile ? "auto" : "visible" }; // scroll only on mobile
  const tableStyle = { width: "100%", borderCollapse: "collapse", minWidth: isMobile ? "700px" : "auto" }; // minWidth ensures scroll on mobile
  ...
  return (
    <main style={{ padding: "2rem", fontFamily: "sans-serif", backgroundColor: "#F9F9F9", minHeight: "100vh" }}>
      ...
      <div style={{ marginBottom: "1rem" }}>
        <button style={{ ...buttonBase, ...(view === "drivers" ? activeBtn : inactiveBtn) }} onClick={() => setView("drivers")}>Drivers</button>
        <button style={{ ...buttonBase, ...(view === "constructors" ? activeBtn : inactiveBtn) }} onClick={() => setView("constructors")}>Constructors</button>
      </div>
